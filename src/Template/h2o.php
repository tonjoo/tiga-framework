...
class h2o
{
    ...
    public static function parseString($source, $options = array())
    {
        $instance = new self(null, array_merge($options, array('loader' => false)));
        $instance->nodelist = $instance->parse($source);

        return $instance;
    }

    public static function &createTag($tag, $args = null, $parser, $position = 0)
    {
     ...
     * @return unknown
     */
    public static function addFilter($filter, $callback = null)
    {
        if (is_array($filter)) {
            $filters = $filter;
    ...
    public static function addLookup($callback)
    {
        if (is_callable($callback)) {
    ...
    public static function load($extension, $file = null)
    {
        if (!$file) {
